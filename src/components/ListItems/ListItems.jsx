...
const ListItems = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:4000"}/api/items`);
        setItems(data);
      } catch (err) {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:4000"}/api/items/${itemId}`);
